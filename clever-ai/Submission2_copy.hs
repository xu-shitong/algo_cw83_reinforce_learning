...
data AIState = AIState
  ...
  , ranks :: PlanetRanks
  , strategyPoints :: Map PlanetId PlanetRanks
  , params :: [Double]
  } deriving Generic
...
initialState = AIState
  ...
  , params = [6.300000, 3.800000, 0.440000, 0.900000, -0.000002]
  }
...
skynet :: GameState -> AIState
       -> ([Order], Log, AIState)
skynet g ai
  | M.null (ranks ai) = skynet g ai {ranks = calRank, strategyPoints = }
  ...
  where 
    ...
    dijCalStrategy :: PlanetId -> PlanetRanks
    dijCalStrategy pId 
      = [ (head . ((Path _ es) -> (\(_, w) -> target w) (last es)) $ paths , sum [ w | p@((Path w _)) <- paths]) | paths <- firstMove ]
      where 
        ps = shortestPaths g pId 
        firstMove = groupBy (\(Path _ es1) (Path _ es2) -> (last es1) == (last es2)) ps
...
applyParams pId g ai 
  = [ (pId,
      ...
      pPr * pr -
      pT * t) | (pId, wId, gro, (PlanetRank pr), t) <- adjList]
  where 
