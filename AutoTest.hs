...
import Submission2
import Test (runTests, test, testFromFile, Test (..))
...
main :: IO ()
main = runTests $ do
  testFromFile "PageRank tests" (pageRank' @PlanetId @GameState) same "tests/pageRankTests.txt"
  testFromFile "PlanetRank tests" planetRank same "tests/planetRankTests.txt"
  testFromFile "Find enemy tests" findEnemyPlanet (==) "tests/findEnemyTests.txt"
  testFromFile "Attack from all tests" (uncurry attackFromAll) sameOrders "tests/attackFromAllTests.txt"
  testFromFile "Send tests" (uncurry3 send) sameOrders "tests/sendTests.txt"
  test "AIState rushTarget tests" (const hasRushTargetField) (==) [() :=> True]
